...
import { collection, query, where, getDocs, onSnapshot, limit, orderBy, addDoc, serverTimestamp } from "firebase/firestore";
import { db } from "../lib/firebase";
import { Heart, DollarSign, ShieldCheck, LogOut, UserCheck, Lock, Activity, ChevronRight } from "lucide-react";
const auth = getAuth();
export default function AdminDashboard() {
    ...
    if (!user) {
        ...
        <form onSubmit={handleLogin} className="glass p-12 space-y-8 text-center max-w-md w-full rounded-[3rem] border border-white/5 shadow-glow">
          ...
          <div className="space-y-4">
            <input type="email" placeholder="E-mail" required className="glass w-full p-5 rounded-2xl outline-none focus:border-destaque/30 transition-all" onChange={(e) => setLoginData({ ...loginData, email: e.target.value })}/>
            <input type="password" placeholder="Senha" required className="glass w-full p-5 rounded-2xl outline-none focus:border-destaque/30 transition-all" onChange={(e) => setLoginData({ ...loginData, senha: e.target.value })}/>
          </div>
    ...
    return (<div className="min-h-screen bg-background text-white p-6 pt-32 font-body">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {["Dev", "Apostolo", "Tesouraria"].includes(user.cargo) ? (<div className="glass p-10 rounded-[3.5rem] relative overflow-hidden group border border-white/5">
                ...
                </div>) : (<div className="glass p-10 rounded-[3.5rem] border border-white/5 flex flex-col justify-center items-center opacity-40 grayscale">
            ...
            <div className="glass p-10 rounded-[3.5rem] border border-white/5 space-y-8">
               ...
               <div className="grid gap-4">
                 {pedidos.map(p => (<div key={p.id} className="p-6 bg-white/5 rounded-[2rem] border border-white/5 flex justify-between items-center group">
          ...
          <div className="space-y-10">
            {["Dev", "Apostolo"].includes(user.cargo) && (<div className="glass p-10 rounded-[3.5rem] border border-white/5 shadow-2xl">
                ...
                <div className="space-y-5">
                  {logsAcesso.map((log) => (<div key={log.id} className="flex items-center gap-4 p-5 bg-white/5 rounded-3xl border border-white/5">
