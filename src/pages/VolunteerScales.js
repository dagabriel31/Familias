...
export default function VolunteerScales({ userRole }) {
    const [servosDisponiveis, setServosDisponiveis] = useState([]);
    ...
    const [servosSelecionados, setServosSelecionados] = useState([]);
    ...
    useEffect(() => {
        ...
        const unsubServos = onSnapshot(collection(db, "usuarios"), (snap) => {
            setServosDisponiveis(snap.docs.map(d => ({ id: d.id, ...d.data() })));
        });
    ...
    const salvarEscala = async (e) => {
        e.preventDefault();
        if (!dataCulto || !minId || servosSelecionados.length === 0)
            return;
    ...
    const toggleServo = (servo) => {
        setServosSelecionados(prev => prev.find(s => s.id === servo.id)
    ...
    return (<div className="min-h-screen bg-background text-white p-6 pt-32 font-body">
      <div className="container mx-auto space-y-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          {/* FORMULÁRIO */}
          {podeGerenciar && (<div className="lg:col-span-1">
              <div className="glass p-10 rounded-[3.5rem] border border-destaque/20 sticky top-32">
                ...
                <form onSubmit={salvarEscala} className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <input type="date" value={dataCulto} onChange={e => setDataCulto(e.target.value)} className="glass w-full p-4 rounded-2xl outline-none focus:border-destaque/50"/>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="glass p-4 rounded-2xl max-h-60 overflow-y-auto space-y-2">
                      {servosDisponiveis.map(s => (<div key={s.id} onClick={() => toggleServo(s)} className={`p-3 rounded-xl cursor-pointer transition-all flex justify-between items-center ${servosSelecionados.find(sel => sel.id === s.id) ? 'bg-destaque text-primaria' : 'hover:bg-white/5 text-white/40'}`}>
                          <div>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {escalas.map(escala => (<div key={escala.id} className="glass p-8 rounded-[3rem] border border-white/5 group">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {escala.servosNomes?.map((nome, idx) => (<div key={idx} className="bg-white/5 px-3 py-2 rounded-full border border-white/5 flex items-center gap-2">
