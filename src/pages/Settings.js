...
export default function ConfigSettings() {
    ...
    useEffect(() => {
        ...
        const fetchConfig = async () => {
            ...
            if (docSnap.exists())
                setHoraNotificacao(docSnap.data().hora);
        };
        fetchConfig();
    }, []);
    ...
    return (<div className="p-8 glass rounded-[3rem] border border-white/5 space-y-8">
      ...
      <div className="space-y-6">
        ...
        <select value={horaNotificacao} onChange={(e) => setHoraNotificacao(e.target.value)} className="w-full glass p-5 rounded-2xl outline-none focus:border-destaque/50 text-xl font-bold">
        ...
        <button onClick={salvarConfig} disabled={loading} className="w-full bg-destaque text-primaria py-5 rounded-full font-black uppercase text-xs tracking-[0.2em] hover:scale-105 transition-all flex items-center justify-center gap-3">
