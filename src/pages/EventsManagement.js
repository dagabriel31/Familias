...
export default function EventsManagement({ userRole }) {
    ...
    return (<div className="min-h-screen bg-background p-6 pt-32 text-white font-body">
      <div className="container mx-auto space-y-12">
        ...
        {podeGerir && (<section className="max-w-4xl mx-auto glass p-10 rounded-[3rem] border border-destaque/20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {eventos.map(ev => (<div key={ev.id} className="glass rounded-[2.5rem] overflow-hidden group border border-white/5">
              <div className="h-48 overflow-hidden relative">
                ...
                {podeGerir && (<button onClick={() => deleteDoc(doc(db, "eventos", ev.id))} className="absolute top-4 right-4 p-3 glass rounded-full text-red-400 hover:bg-red-500 hover:text-white transition-all">
                    <Trash2 size={16}/>
                  </button>)}
              </div>
