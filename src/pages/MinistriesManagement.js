...
import { Shield, LayoutGrid, Info, CheckCircle2 } from "lucide-react";
export default function MinistriesManagement({ userRole }) {
    ...
    const atualizarDescricao = async (id, novaDesc) => {
        setSalvando(id);
        try {
    ...
    return (<div className="min-h-screen bg-background p-6 pt-32 text-white font-body">
      <div className="container mx-auto space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {ministerios.map(m => (<div key={m.id} className="glass p-1 rounded-[3.5rem] border border-white/5 relative group overflow-hidden">
              {/* Imagem com Overlay */}
              ...
              <div className="p-8 pt-4 space-y-6">
                <div className="flex justify-between items-center">
                  ...
                  {salvando === m.id ? (<CheckCircle2 size={16} className="text-green-400 animate-pulse"/>) : (<LayoutGrid size={16} className="text-white/20"/>)}
                </div>

                {podeGerir ? (<div className="space-y-2">
                    <label className="text-[9px] uppercase tracking-widest text-white/30 font-bold">Descrição Editável</label>
                    <textarea defaultValue={m.desc} onBlur={(e) => atualizarDescricao(m.id, e.target.value)} className="w-full bg-white/5 border border-white/10 p-4 rounded-2xl text-sm italic text-white/80 resize-none outline-none focus:border-destaque/40 h-32 transition-all" placeholder="Descreva a missão deste ministério..."/>
                  </div>) : (<p className="text-sm text-white/60 leading-relaxed italic border-l-2 border-destaque/30 pl-4">
                    "{m.desc}"
                  </p>)}
